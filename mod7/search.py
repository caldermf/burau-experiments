...
DEFAULT_USE_BEST_MILLIONS = 7.0
DEFAULT_BUCKET_CAP_MILLIONS = 12.0
DEFAULT_MAX_STEPS = 170

MAX_STEPS = DEFAULT_MAX_STEPS
USE_BEST = int(DEFAULT_USE_BEST_MILLIONS * 1_000_000)
OUTPUT_CAP = 8 * USE_BEST
BUCKET_CAP = int(DEFAULT_BUCKET_CAP_MILLIONS * 1_000_000)
...
def configure_search(
    use_best_millions=DEFAULT_USE_BEST_MILLIONS,
    bucket_cap_millions=DEFAULT_BUCKET_CAP_MILLIONS,
    max_steps=DEFAULT_MAX_STEPS,
):
    """Configure host-side frontier sizes before launching the search."""
    global USE_BEST, OUTPUT_CAP, BUCKET_CAP, MAX_STEPS

    if use_best_millions <= 0:
        raise ValueError("use_best_millions must be positive")
    if bucket_cap_millions <= 0:
        raise ValueError("bucket_cap_millions must be positive")
    if max_steps < 0:
        raise ValueError("max_steps must be nonnegative")

    USE_BEST = int(use_best_millions * 1_000_000)
    OUTPUT_CAP = 8 * USE_BEST
    BUCKET_CAP = int(bucket_cap_millions * 1_000_000)
...
# ==============================================================================
#
# Static suffix descriptors and seed matrices live in suffix_data.py. The kernel
# still receives each suffix descriptor as tl.constexpr values, so this extraction
# is just readability cleanup; it should not change Triton specialization.
...
def save_projlen0_braids(zero_data_soa, zero_meta, zero_words, braid_length, save_dir=None):
    """
...
def default_save_dir():
    """Return the package-local result directory for projlen-0 candidates."""
    return os.path.join(os.path.dirname(os.path.abspath(__file__)), "results", "projlen0")


def run_search(save_dir=None):
    """
    Main search loop.
    Runs from braid length 1 (the 22 seeds) through length MAX_STEPS + 1.
    ...
    print(f"Layout: SoA [54, N], one Triton suffix kernel launched in random suffix order")
    print("=" * 100)
    ...
    if n_seed_zeros > 0:
        ...
        torch.save({
            ...
            "braid_length": 1,
        }, os.path.join(save_dir, "projlen0_length001.pt"))
    ...
    if total_projlen0 > 0:
        files = sorted(glob.glob(os.path.join(save_dir, "projlen0_length*.pt")))
        for f in files:
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description="Standalone mod-7 GPU search for low-projlen B_4 Burau candidates"
    )
    parser.add_argument(
        "-u",
        "--use-best",
        type=float,
        default=DEFAULT_USE_BEST_MILLIONS,
        help="parents kept per generation, in millions",
    )
    parser.add_argument(
        "-b",
        "--bucket-cap",
        type=float,
        default=DEFAULT_BUCKET_CAP_MILLIONS,
        help="maximum accepted children per projlen bucket, in millions",
    )
    parser.add_argument(
        "-m",
        "--max-steps",
        ...
        help="number of expansion steps after the length-1 seed frontier",
    )
    parser.add_argument(
        "--save-dir",
        default=default_save_dir(),
        help="directory for saved projlen-0 .pt files",
    )
...
def main(argv=None):
    ...
    configure_search(
        use_best_millions=args.use_best,
        bucket_cap_millions=args.bucket_cap,
        max_steps=args.max_steps,
    )
    run_search(save_dir=args.save_dir)
