...
Uses gpu_tracker_v2.py which correctly interfaces with peyl/jonesrep.py.

Usage:
    # Basic search for n=4, p=5
    python run_search_v2.py --p 5

    # Search with different parameters
    python run_search_v2.py --p 7 --bucket-size 1000 --max-length 100

    # Run on specific GPU
    CUDA_VISIBLE_DEVICES=0 python run_search_v2.py --p 5

    # Multiple parallel runs (recommended for stochastic search)
    for i in {1..8}; do
        python run_search_v2.py --p 5 --seed $i --output results_$i.json &
    done
"""
...
def parse_args():
    ...
    parser.add_argument("--test", action="store_true",
                       help="Run quick test instead of full search")
...
def run_test():
    ...
    tracker = GPUTracker(rep, bucket_size=50, seed=42)

    # Quick bootstrap
    tracker.bootstrap_exhaustive(upto_length=4)
    ...
    print("\nMin projlen by length:")
    for length in sorted(stats.min_projlen_at_length.keys()):
...
def main():
    ...
    print("="*60)
    print("GPU-Accelerated Burau Kernel Element Search (v2)")
    print("="*60)
    print()
    ...
    print(f"  Bootstrap length: {args.bootstrap_length}")
    ...
    try:
        ...
        print("Please ensure peyl is in the Python path")
        return 1
