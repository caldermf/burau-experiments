#!/usr/bin/env python3
"""
Find kernel elements for various primes p, with memory profiling.
...
def find_kernel(
    ...
    # Set up profiler
    profiler = None
    if profile and config.device == "cuda":
        try:
            ...
            print("Warning: memory_profiler module not found, profiling disabled")
            profile = False
    elif profile and config.device != "cuda":
        print("Warning: Memory profiling only available on CUDA devices")
        profile = False
...
Quick profiling (run only a few levels to check memory):
  ...
  %(prog)s --p 5 --bucket-size 200000 --max-length 150 --runs 15 --profile
    ...
    parser.add_argument("--profile", action="store_true",
                        help="Enable memory profiling (generates .pickle for memviz + .csv)")
