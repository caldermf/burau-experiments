"""
MAXIMALLY OPTIMIZED braid search v2 with:
...
3. REDUCED GC OVERHEAD - only gc every N chunks
4. FUSED projlen computation into sampling
5. LARGER chunks to reduce loop overhead
6. Better timing instrumentation

Expected speedup: 2-4x over v1 (ultra) implementation
"""
...
class Config:
    ...
    gc_every_n_chunks: int = 10  # NEW: Only GC every N chunks
...
def compute_projlen_batch(matrices: torch.Tensor) -> torch.Tensor:
    """Compute projective length for a batch of matrices - OPTIMIZED."""
    N, _, _, D = matrices.shape
    device = matrices.device
    
    # Reshape once: (N, 9, D)
    flat_by_degree = matrices.reshape(N, 9, D)
    
    # Check which degrees have any nonzero entry across all 9 matrix elements
    degree_has_nonzero = (flat_by_degree != 0).any(dim=1)  # (N, D)
    
    # Find min and max nonzero degrees
    # Use cumsum trick for efficiency
    has_any_nonzero = degree_has_nonzero.any(dim=1)  # (N,)
    
    # For matrices with nonzeros: find extent
    min_degrees = degree_has_nonzero.int().argmax(dim=-1)  # First nonzero
    max_degrees = D - 1 - degree_has_nonzero.flip(dims=[-1]).int().argmax(dim=-1)  # Last nonzero
    
    projlens = max_degrees - min_degrees + 1
    projlens = torch.where(has_any_nonzero, projlens, torch.zeros_like(projlens))
...
# =============================================================================
# GPU BUCKETS - OPTIMIZED
# =============================================================================

class GPUBuckets:
    """Maintains reservoir-sampled buckets entirely on GPU - OPTIMIZED."""
    ...
    def __init__(self, bucket_size: int, device: torch.device):
        ...
        # Pre-allocate a buffer for projlen counting
        self._projlen_counts: dict[int, int] = {}
    
    def add_chunk_with_projlen(
        self,
        ...
        """
        Add chunk AND compute projlen in one pass.
        Returns projlen counts for this chunk.
        """
        if len(matrices) == 0:
            return {}
        
        # Compute projlen INSIDE this method to fuse operations
        ...
        kernel_mask = (projlens == 1)
        kernel_words = None
        if kernel_mask.any():
            kernel_words = words[kernel_mask].clone()
        
        priorities = torch.rand(len(matrices), device=self.device)
        
        # Get unique projlens and their counts efficiently
        unique_pls, inverse, counts = torch.unique(projlens, return_inverse=True, return_counts=True)
        
        # Build projlen_counts dict
        projlen_counts = {}
        for i, (pl, c) in enumerate(zip(unique_pls.tolist(), counts.tolist())):
            projlen_counts[pl] = c
        
        # Process each unique projlen
        for i, pl in enumerate(unique_pls.tolist()):
            mask = (inverse == i)
        ...
        return projlen_counts, kernel_words
...
class BraidSearchUltra:
    ...
    def initialize(self):
        ...
        print(f"Config: bucket_size={self.config.bucket_size}, "
              ...
              f"gc_every={self.config.gc_every_n_chunks}")
        print(f"⚡ Ultra-optimized v2: reduced GC, fused projlen")
    ...
    def process_level(self, level: int):
        ...
        t_gather = time.time() - t_gather_start
        print(f"  Starting braids: {num_starting} (gather: {t_gather:.2f}s)")
        ...
        print(f"  Candidates to generate: {num_candidates} (expand_idx: {t_expand:.2f}s)")
        ...
        gc_every = self.config.gc_every_n_chunks
        
        for chunk_idx in range(num_chunks):
            # ONLY GC every N chunks instead of every chunk!
            if chunk_idx > 0 and chunk_idx % gc_every == 0:
                gc.collect()
            ...
            # FUSED: projlen computation + bucket insertion
            t0 = time.time()
            chunk_projlen_counts, kernel_words = gpu_buckets.add_chunk_with_projlen(
                chunk_matrices, chunk_words, chunk_lengths,
                is_bootstrap
            )
            t_sample_total += time.time() - t0
            
            # Handle kernel elements
            if kernel_words is not None and len(kernel_words) > 0:
                num_ones = len(kernel_words)
                print(f"\n  🎉 FOUND {num_ones} KERNEL ELEMENTS (projlen=1)! 🎉")
                self.kernel_braids.append(kernel_words.cpu())
            
            # Accumulate projlen counts (still needed for stats)
            for pl, c in chunk_projlen_counts.items():
                projlen_counts[pl] = projlen_counts.get(pl, 0) + c
            
            del chunk_matrices, chunk_words, chunk_lengths
        ...
        t_other = level_time - t_matmul_total - t_sample_total - t_gather - t_expand
        print(f"  ⚡ Timing: matmul={t_matmul_total:.2f}s, sample+projlen={t_sample_total:.2f}s, "
              f"gather={t_gather:.2f}s, expand={t_expand:.2f}s, other={t_other:.2f}s, total={level_time:.2f}s")
        ...
        self.stats["time_other"].append(t_other)
