...
class Config:
    ...
    use_best: int = 0
    run_levels: int = 0  # NEW: 0 = run to max_length, >0 = stop after this many levels
...
class BraidSearch:
    ...
    v3.5: Added run_levels parameter for controlled stopping.
    """
    ...
    def initialize(self):
        ...
        if self.config.run_levels > 0:
            print(f"         run_levels={self.config.run_levels} (will stop early)")
    ...
    def run(self, checkpoint_dir: Optional[str] = None, resume_from: Optional[str] = None):
        ...
        # Compute stop level
        if self.config.run_levels > 0:
            stop_level = self.start_level + self.config.run_levels - 1
            stop_level = min(stop_level, self.config.max_length)
            print(f"\n⏱ Will stop after level {stop_level} (run_levels={self.config.run_levels})")
        else:
            stop_level = self.config.max_length
        
        try:
            for level in range(self.start_level, stop_level + 1):
            ...
            print(f"\n{'='*60}")
            if final_level == self.config.max_length:
                print("SEARCH COMPLETE")
            elif self.config.run_levels > 0 and final_level == stop_level:
                print(f"SEARCH STOPPED (completed {self.config.run_levels} levels)")
            else:
