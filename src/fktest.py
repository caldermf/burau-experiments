...
def find_kernel(
    ...
    kernel_braids = search.run()
     ...
     # Quick degree check
    if hasattr(search, 'buckets') and search.buckets:
        max_deg = max(
            (m.reshape(-1, m.shape[-1]) != 0).any(0).nonzero()[-1].item() 
            for m, _, _ in search.buckets.values()
        )
        D = config.degree_window
        util = 100 * (max_deg + 1) / D
        recommended = max(1, int(np.ceil(max_deg / config.max_length * 1.2)))
        
        print(f"\nDegree usage: {max_deg}/{D-1} ({util:.1f}% utilization)")
        if recommended < config.degree_multiplier:
            savings = 100 * (1 - (recommended * config.max_length + 1) / D)
            print(f"💡 Can reduce to --degree-multiplier {recommended} (save {savings:.1f}% memory)")
