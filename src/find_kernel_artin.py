...
ULTRA-OPTIMIZED VERSION with precomputed FFTs, non-negative degrees only,
and DEVIATION-BASED OPTIMIZATION targeting specific v^target scalar.

The key insight: kernel elements of the abelianization evaluate to v^d * I where
d = (2/3) * artin_length. By tracking Artin length and optimizing for deviation
from this target (rather than just projlen), we search more efficiently.

Usage examples:
    python find_kernel.py --p 7 --bucket-size 100000 --use-best 100000 --max-length 150 --device cuda
    python find_kernel.py --p 5 --bucket-size 1500000 --use-best 750000 --max-length 60
"""
...
from braid_search_artin import Config, BraidSearchUltra as BraidSearch, load_tables_from_file
...
def verify_kernel_element(word_list, n=4, r=1, p=2, expected_artin_length=None):
    """
    Verify that a braid word is actually in the kernel using peyl.
    
    If expected_artin_length is provided, also check that the scalar power
    matches (2/3) * artin_length.
    """
    if not PEYL_AVAILABLE:
        return True, "Verification skipped (peyl not available)", None
    
    if not word_list:
        return False, "Empty word", None
    
    try:
        braid = GNF(n=n, power=0, factors=tuple(word_list))
    except AssertionError as e:
        return False, f"Invalid normal form: {e}", None
        ...
        for j in range(3):
            if i == j:
                if not np.array_equal(result[i, j, :], diag_poly):
                    return False, f"Diagonal mismatch at [{i},{j}]", None
            else:
                if np.any(result[i, j, :] != 0):
                    return False, f"Off-diagonal nonzero at [{i},{j}]", None
    
    nonzero_degs = np.where(diag_poly != 0)[0]
    if len(nonzero_degs) == 0:
        return True, "Kernel element! Evaluates to 0 (trivial)", 0
    ...
    if len(nonzero_degs) == 1:
        ...
        if expected_artin_length is not None:
            expected_target = (2/3) * expected_artin_length
            if abs(deg - expected_target) < 0.01:  # Should match exactly
                return True, f"Kernel element! Evaluates to ({scalar_str}) * I [matches target {expected_target:.1f}]", deg
            else:
                return True, f"Kernel element! Evaluates to ({scalar_str}) * I [target was {expected_target:.1f}]", deg
        
        return True, f"Kernel element! Evaluates to ({scalar_str}) * I", deg
    else:
        terms = [f"{diag_poly[d]}*v^{d}" for d in nonzero_degs]
        scalar_str = " + ".join(terms)
        return True, f"Kernel element! Evaluates to ({scalar_str}) * I", None


def compute_artin_length_from_word(word_list, simple_artin_lengths):
    """Compute total Artin length from a Garside normal form word."""
    return sum(simple_artin_lengths[s] for s in word_list)
...
def find_kernel(
    ...
    """Search for kernel elements using ultra-optimized algorithm with deviation-based search."""
    ...
    print("="*60)
    print(f"SEARCHING FOR p={p} KERNEL ELEMENTS (DEVIATION-OPTIMIZED)")
    print("="*60)
    ...
    # Get Artin lengths for verification
    artin_lengths_cpu = search.simple_artin_lengths.cpu().numpy()
    ...
    if not kernel_braids:
        print("No deviation=0 braids found.")
        return None
    ...
    for batch_idx, batch in enumerate(kernel_braids):
        ...
        for i, word_tensor in enumerate(batch):
            ...
            # Compute Artin length for this braid
            artin_length = compute_artin_length_from_word(word_list, artin_lengths_cpu)
            expected_target = (2/3) * artin_length
            
            is_kernel, msg, actual_deg = verify_kernel_element(word_list, p=p, expected_artin_length=artin_length)
            
            if is_kernel:
                verified.append((word_list, artin_length, actual_deg))
                print(f"\n  🎉 KERNEL ELEMENT #{len(verified)} 🎉")
                print(f"    Factors: {word_list}")
                print(f"    Garside length: {len(word_list)}")
                print(f"    Artin length: {artin_length}")
                print(f"    Target v-power: {expected_target:.2f}")
                print(f"    {msg}")
                ...
                print(f"  Braid {i}: {word_list[:8]}{'...' if len(word_list) > 8 else ''}")
                print(f"    Artin len={artin_length}, target={expected_target:.2f} - {msg}")
    
    print(f"\n{'='*60}")
    print(f"SUMMARY")
    print("="*60)
    print(f"Total candidates with deviation=0: {sum(len(b) for b in kernel_braids)}")
    print(f"Verified kernel elements: {len(verified)}")
    
    if verified:
        print(f"\n✓ SUCCESS! Found {len(verified)} kernel elements for p={p}")
        
        # Statistics on found elements
        artin_lens = [v[1] for v in verified]
        if artin_lens:
            print(f"  Artin lengths range: [{min(artin_lens)}, {max(artin_lens)}]")
            print(f"  v-powers (targets): {sorted(set((2/3)*a for a in artin_lens))}")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Search for kernel elements (DEVIATION-OPTIMIZED)",
...
The deviation-based search targets v^d * I where d = (2/3) * artin_length.
This is more efficient than plain projlen-based search because it accounts
for the fact that kernel elements (of the abelianization) have a SPECIFIC
power of v determined by their Artin length.
...
Recommended settings for RTX 5000 (32GB):
  --bucket-size 100000 --use-best 100000 --matmul-chunk 20000
    ...
    parser.add_argument("--bucket-size", "-b", type=int, default=4000,
                        help="Number of braids to keep per deviation bucket")
