#!/usr/bin/env bash
# Train a slightly deeper version of the original Burau transformer while
# keeping the same overall hierarchical design.

#SBATCH --job-name=braidmod-xfmr-tuned-deep
...
OUT_DIR="$REPO_ROOT/artifacts/garside_transformer_tuned_deeper_noema_L30to60_p5_D140_N200000_e30"
...
"$PYTHON_PATH" plot_training_curves.py \
  --log "$LOG_PATH" \
  --out "$PLOT_PATH" \
  --title "Tuned Burau Transformer Curves (deeper arch, no EMA, L30to60, p=5, D=140, N=200000, 30 epochs)"
