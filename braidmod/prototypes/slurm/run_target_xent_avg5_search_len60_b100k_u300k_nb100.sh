#!/usr/bin/env bash
# Run pure target-cross-entropy reservoir search with 5-step running-average scoring.

#SBATCH --job-name=braidmod-search-xentavg5-l60-b100k-u300k-nb100
...
OUT_JSON="$REPO_ROOT/artifacts/reservoir_search_target_xentavg5_gpu_len60_b100k_u300k_nb100_boot5.json"
OUT_PNG="$REPO_ROOT/artifacts/reservoir_search_target_xentavg5_gpu_len60_b100k_u300k_nb100_boot5_best_projlen.png"
...
echo "Starting target_xent avg5 search at $(date)"
echo "Host: $(hostname)"
...
out_json = Path("artifacts/reservoir_search_target_xentavg5_gpu_len60_b100k_u300k_nb100_boot5.json")
out_png = Path("artifacts/reservoir_search_target_xentavg5_gpu_len60_b100k_u300k_nb100_boot5_best_projlen.png")
payload = json.loads(out_json.read_text(encoding="utf-8"))
levels = [int(item["level"]) for item in payload["level_summaries"]]
best_projlen = [int(item["best_candidate"]["garside_length"] and item["best_candidate"]["score"] is not None or 0) for item in payload["level_summaries"]]

# Plot the actual best projlen found at each level from the saved candidate factors.
...
ax.set_title("Best Projlen by Length (target_xent avg5 search)")
