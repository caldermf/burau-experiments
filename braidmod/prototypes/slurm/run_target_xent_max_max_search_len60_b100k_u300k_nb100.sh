#!/usr/bin/env bash
# Run pure target-cross-entropy reservoir search with max-so-far scoring, maximizing target_xent.

#SBATCH --job-name=braidmod-search-xentmaxmax-l60-b100k-u300k-nb100
...
OUT_JSON="$REPO_ROOT/artifacts/reservoir_search_target_xentmaxmax_gpu_len60_b100k_u300k_nb100_boot5.json"
OUT_PNG="$REPO_ROOT/artifacts/reservoir_search_target_xentmaxmax_gpu_len60_b100k_u300k_nb100_boot5_best_projlen.png"
...
echo "Starting target_xent max-so-far maximize search at $(date)"
echo "Host: $(hostname)"
...
"$PYTHON_PATH" -u reservoir_search_braidmod.py \
  ...
  --bootstrap-length 5 \
  --num-buckets 100 \
  --score-type target_xent_max_maximize \
...
out_json = Path("artifacts/reservoir_search_target_xentmaxmax_gpu_len60_b100k_u300k_nb100_boot5.json")
out_png = Path("artifacts/reservoir_search_target_xentmaxmax_gpu_len60_b100k_u300k_nb100_boot5_best_projlen.png")
...
ax.set_title("Best Projlen by Length (target_xent max-so-far maximize search)")
