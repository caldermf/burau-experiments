#!/usr/bin/env bash
# Render two-line average overlays for the tuned factor-only transformer using
# the first five saved kernel hits versus the saved random suite.

#SBATCH --job-name=braidmod-xent-avgcurves
...
echo "Starting averaged two-line overlay render at $(date)"
echo "Host: $(hostname)"
...
for window in 5 7 10 13 15 20; do
  "$PYTHON_PATH" -u render_average_kernel_random_xent_overlay.py \
    ...
    --suite-dir "$SUITE_DIR" \
    --out-png "$SUITE_DIR/kernel_avg_first5_vs_random_avg_target_xent_avg${window}_overlay.png" \
    --device cuda \
    --mode avg5 \
    --window "$window" \
    --max-length 60 \
...
ls -lh "$SUITE_DIR"/kernel_avg_first5_vs_random_avg_target_xent_avg*_overlay.png
