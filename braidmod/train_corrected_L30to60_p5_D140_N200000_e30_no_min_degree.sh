#!/usr/bin/env bash
# Train the corrected/projective Burau final-factor MLP on the L30..60, p=5, D=140, N=200000 dataset
# for 30 epochs, without the projective min-degree feature.

#SBATCH --job-name=braidmod-train-p5-L30to60-D140-e30-nomin
...
OUT_DIR="$REPO_ROOT/artifacts/garside_mlp_L30to60_p5_D140_N200000_e30_no_min_degree"
...
echo "Starting no-min-degree training at $(date)"
echo "Host: $(hostname)"
...
"$PYTHON_PATH" plot_training_curves.py \
  --log "$LOG_PATH" \
  --out "$PLOT_PATH" \
  --title "Corrected Burau Training Curves (L30to60, p=5, D=140, N=200000, 30 epochs, no min degree)"
