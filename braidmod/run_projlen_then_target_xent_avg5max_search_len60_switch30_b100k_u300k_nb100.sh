#!/usr/bin/env bash
# Run projlen search through length 30, then switch to maximizing avg5 target_xent.

#SBATCH --job-name=braidmod-search-proj2xentavg5max-l60-s30-b100k-u300k-nb100
...
OUT_JSON="$REPO_ROOT/artifacts/reservoir_search_projlen_then_target_xentavg5max_gpu_len60_switch30_b100k_u300k_nb100_boot5.json"
OUT_PNG="$REPO_ROOT/artifacts/reservoir_search_projlen_then_target_xentavg5max_gpu_len60_switch30_b100k_u300k_nb100_boot5_best_projlen.png"
...
echo "Starting projlen-then-target_xent avg5 maximize search at $(date)"
echo "Host: $(hostname)"
...
"$PYTHON_PATH" -u reservoir_search_braidmod.py \
  ...
  --bootstrap-length 5 \
  --num-buckets 100 \
  --score-type projlen_then_target_xent_maximize \
  --score-switch-length 30 \
...
out_json = Path("artifacts/reservoir_search_projlen_then_target_xentavg5max_gpu_len60_switch30_b100k_u300k_nb100_boot5.json")
out_png = Path("artifacts/reservoir_search_projlen_then_target_xentavg5max_gpu_len60_switch30_b100k_u300k_nb100_boot5_best_projlen.png")
...
ax.axvline(30, color="#aa3a2a", linestyle="--", linewidth=1.2)
ax.set_xlabel("Garside Length")
ax.set_ylabel("Best Projlen Found")
ax.set_title("Best Projlen by Length (projlen until 30, then target_xent avg5 maximize)")
