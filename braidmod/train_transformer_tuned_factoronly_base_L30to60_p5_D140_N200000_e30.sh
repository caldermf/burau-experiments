#!/usr/bin/env bash
# Train the original Burau transformer directly on final-factor prediction
# with tuned regularization and loss-based checkpoint selection.

#SBATCH --job-name=braidmod-xfmr-factor-base
...
OUT_DIR="$REPO_ROOT/artifacts/garside_transformer_tuned_factoronly_base_noema_L30to60_p5_D140_N200000_e30"
...
"$PYTHON_PATH" plot_training_curves.py \
  --log "$LOG_PATH" \
  --out "$PLOT_PATH" \
  --title "Tuned Burau Transformer Curves (factor-only base, no EMA, L30to60, p=5, D=140, N=200000, 30 epochs)"
