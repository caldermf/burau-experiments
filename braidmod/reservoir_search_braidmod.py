...
def _build_initial_frontier(config: SearchConfig, device: torch.device) -> FrontierBatch:
    ...
    return FrontierBatch(tensors=tensors, words=words, lengths=lengths, last_factor_ids=last_factor_ids)
...
def _monomial_target_match_batch(
    ...
    support_degrees = degree_has_nonzero.int().argmax(dim=-1)
    batch_idx = torch.arange(num_items, device=tensors.device)
    coeff_slices = tensors[batch_idx, support_degrees].to(torch.long) % p
...
class BaseScoreFunction:
    ...
    def score_batch(self, tensors: torch.Tensor) -> ScoreBatch:
    ...
    def bucketize(self, bucket_scores: torch.Tensor, num_buckets: int) -> torch.Tensor:
        ...
        active_buckets = min(num_buckets, num_items)
        order = torch.argsort(bucket_scores, dim=0, descending=False)
        ranks = torch.empty_like(order)
        ranks[order] = torch.arange(num_items, device=bucket_scores.device, dtype=order.dtype)

        bucket_ids = torch.div(ranks * active_buckets, num_items, rounding_mode="floor")
        return torch.clamp(bucket_ids.to(torch.long), 0, active_buckets - 1)


class ProjlenScoreFunction(BaseScoreFunction):
    def __init__(self, device: torch.device, search_D: int):
    ...
    def score_batch(self, tensors: torch.Tensor) -> ScoreBatch:
        ...
        bucket_scores = (projlens.to(SCORE_DTYPE) - 1.0) / float(denom)
        bucket_scores = torch.clamp(bucket_scores, 0.0, 1.0)
...
class ModelConfusionScoreFunction(BaseScoreFunction):
    def __init__(self, checkpoint_path: str, device: torch.device, score_chunk_size: int, expected_p: int):
        super().__init__(device)
...
class HybridScoreFunction(BaseScoreFunction):
    def __init__(
        ...
        super().__init__(device)
        self.projlen = ProjlenScoreFunction(device=device, search_D=search_D)
    ...
    def score_batch(self, tensors: torch.Tensor) -> ScoreBatch:
        proj = self.projlen.score_batch(tensors)
        conf = self.confusion.score_batch(tensors)
        combined = (
...
class GPUBuckets:
    ...
    def add_chunk(
        ...
        for bucket_id in unique_ids.tolist():
            ...
            if bucket_id in self.data:
                old_tensors, old_words, old_lengths, old_last, old_scores, old_priorities = self.data[bucket_id]
    ...
    def materialize(self) -> FrontierBatch:
        ...
        words = torch.cat([entry[1][1] for entry in ordered], dim=0)
        lengths = torch.cat([entry[1][2] for entry in ordered], dim=0)
        last_factor_ids = torch.cat([entry[1][3] for entry in ordered], dim=0)
        scores = torch.cat([entry[1][4] for entry in ordered], dim=0)
...
class ReservoirSearchBraidmod:
    ...
    def _build_scorer(self) -> BaseScoreFunction:
        if self.config.score_type == "projlen":
            return ProjlenScoreFunction(device=self.device, search_D=self.config.search_D)
    ...
    def process_level(self, level: int) -> bool:
        ...
        for start in range(0, num_candidates, self.config.expansion_chunk_size):
            ...
            t0 = time.time()
            child_tensors = _right_multiply_simple_batch(
            ...
            t0 = time.time()
            score_batch = self.scorer.score_batch(child_tensors)
            bucket_ids = self.scorer.bucketize(score_batch.bucket_scores, self.config.num_buckets)
            ...
            score_sum += float(score_batch.raw_scores.sum().item())
...
def parse_args():
    ...
    parser.add_argument("--search-D", type=int, help="Degree window for the search tensors; default is 4*max_length+1")
    ...
    parser.add_argument(
        "--score-type",
        choices=("projlen", "confusion", "hybrid"),
        default="projlen",
        help="Scoring function to minimize",
    )
    parser.add_argument("--checkpoint", help="Model checkpoint for confusion or hybrid scoring")
    parser.add_argument("--projlen-weight", type=float, default=1.0, help="Hybrid score weight on normalized projlen")
    parser.add_argument("--confusion-weight", type=float, default=1.0, help="Hybrid score weight on confusion")
