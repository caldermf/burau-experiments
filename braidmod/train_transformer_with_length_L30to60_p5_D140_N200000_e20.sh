#!/usr/bin/env bash
# Train the hierarchical Burau transformer with true Garside length conditioning
# on the corrected/projective L30..60, p=5, D=140, N=200000 dataset for 20 epochs.

#SBATCH --job-name=braidmod-xfmr-len-p5-L30to60-D140-e20
...
OUT_DIR="$REPO_ROOT/artifacts/garside_transformer_with_length_L30to60_p5_D140_N200000_e20"
...
"$PYTHON_PATH" plot_training_curves.py \
  --log "$LOG_PATH" \
  --out "$PLOT_PATH" \
  --title "Burau Transformer + Length Training Curves (L30to60, p=5, D=140, N=200000, 20 epochs)"
