"""
GENERALIZED GPU-accelerated braid search for kernel elements.

Supports arbitrary:
- n: number of strands in B_n
- r: partition parameter for two-row representation (n-r, r)
- p: prime for modular reduction

Key optimizations:
1. Batched FFT matrix multiplication
2. Precomputed FFTs of simple matrices
...
class Config:
    ...
    # Braid group parameters
    n: int = 4                    # Number of strands (B_n)
    r: int = 1                    # Partition (n-r, r), r >= 1
    ...
    @property
    def num_simples(self) -> int:
        """Number of simple elements = n!"""
        return math.factorial(self.n)
    
    @property
    def rep_dim(self) -> int:
        """Dimension of the (n-r, r) representation."""
        if self.r == 0:
            return 1
    ...
    def __post_init__(self):
        assert self.n >= 2, f"Need n >= 2, got {self.n}"
        assert self.r >= 1, f"Need r >= 1, got {self.r}"
        assert self.n - 2 * self.r >= 0, f"Need n >= 2r, got n={self.n}, r={self.r}"
...
# =============================================================================
# OPTIMIZED POLYNOMIAL MATRIX MULTIPLICATION
# =============================================================================

class FastPolyMatmul:
    """
    Precomputes and caches FFTs of simple matrices for maximum speed.
    Generalized for arbitrary matrix dimensions.
    """
    
    def __init__(self, simple_matrices: torch.Tensor, D: int, dim: int, device: torch.device):
        """
        Args:
            simple_matrices: (num_simples, dim, dim, D) tensor
            D: degree window size
            dim: matrix dimension (rep_dim)
            device: torch device
        """
        self.D = D
        self.dim = dim
        ...
        print(f"Precomputing FFTs of simple matrices...")
        print(f"  dim={dim}, D={D}, out_D={self.out_D}, fft_size={self.fft_size}")
        
        self.simple_matrices = simple_matrices.to(device)
        self.simple_matrices_fft = torch.fft.rfft(
            simple_matrices.float().to(device),
            n=self.fft_size,
            dim=-1
        )
        
        fft_mem = self.simple_matrices_fft.numel() * 8 / 1e6
    ...
    def matmul_batch(self, A: torch.Tensor, suffix_indices: torch.Tensor, 
        ...
        N = A.shape[0]
        C = torch.zeros(N, self.dim, self.dim, self.out_D, 
                        dtype=COMPUTE_DTYPE_INT, device=self.device)
        ...
        for start in range(0, N, chunk_size):
            ...
            B_fft = self.simple_matrices_fft[chunk_indices]
            
            # General matrix multiply via einsum
...
def compute_projlen_batch(matrices: torch.Tensor) -> torch.Tensor:
    """Compute projective length for a batch of matrices of any dimension."""
    N, dim, _, D = matrices.shape
    ...
    for start in range(0, N, sub_batch_size):
        ...
        # Reshape to (batch_N, dim^2, D) for degree analysis
        by_degree = batch.reshape(batch_N, dim * dim, D)
...
def is_scalar_identity_batch(matrices: torch.Tensor) -> torch.Tensor:
    ...
    for start in range(0, N, sub_batch_size):
        ...
        off_diag_zero = torch.ones(batch_N, dtype=torch.bool, device=device)
        for i in range(dim):
            for j in range(dim):
                if i != j:
                    off_diag_zero &= (batch[:, i, j, :] == 0).all(dim=-1)
        
        # Check all diagonal entries are equal to the first
        diag_00 = batch[:, 0, 0, :]
        diag_equal = torch.ones(batch_N, dtype=torch.bool, device=device)
        for i in range(1, dim):
            diag_equal &= (batch[:, i, i, :] == diag_00).all(dim=-1)
...
class BraidSearchUltra:
    ...
    Generalized for arbitrary n and partition (n-r, r).
    """
    
    def __init__(
        self,
        simple_matrices: torch.Tensor,
        valid_suffixes: torch.Tensor,
        num_valid_suffixes: torch.Tensor,
        config: Config
    ):
        ...
        self.dim = config.rep_dim
        
        self.simple_matrices = simple_matrices.to(STORAGE_DTYPE_MATRIX).to(self.device)
        self.valid_suffixes = valid_suffixes.to(self.device)
        self.num_valid_suffixes = num_valid_suffixes.to(self.device)
        
        self.D = simple_matrices.shape[-1]
        
        self.fast_matmul = FastPolyMatmul(simple_matrices, self.D, self.dim, self.device)
    ...
    def initialize(self):
        ...
        identity_matrix = torch.zeros(1, self.dim, self.dim, self.D, 
                                       dtype=STORAGE_DTYPE_MATRIX, device=self.device)
        for i in range(self.dim):
            identity_matrix[0, i, i, 0] = 1  # v^0 = 1 at index 0
        
        identity_word = torch.zeros(1, self.config.max_length, 
        ...
        print(f"Braid group: B_{self.config.n}")
        print(f"Representation: ({self.config.n - self.config.r}, {self.config.r}), dimension {self.dim}")
        ...
        print(f"⚡ Ultra-optimized: precomputed FFTs + non-negative degrees only")
    ...
    def expand_and_multiply_chunk(
        ...
        """Expand a chunk using PRECOMPUTED FFTs of simple matrices."""
...
def load_tables_from_file(config: Config, table_path: str):
    ...
    assert tables['n'] == config.n, f"Table n={tables['n']} != config n={config.n}"
    assert tables['r'] == config.r, f"Table r={tables['r']} != config r={config.r}"
    assert tables['p'] == config.prime, f"Table p={tables['p']} != config prime={config.prime}"
    ...
    simple_matrices = torch.zeros(num_simples, dim, dim, D, dtype=STORAGE_DTYPE_MATRIX)
    ...
    for s in range(num_simples):
        ...
        if dst_end <= D:
            simple_matrices[s, :, :, dst_start:dst_end] = mat[:, :, src_start:src_end].to(STORAGE_DTYPE_MATRIX)
        else:
            usable_end = D
            src_usable_end = src_start + (usable_end - dst_start)
            simple_matrices[s, :, :, dst_start:usable_end] = mat[:, :, src_start:src_usable_end].to(STORAGE_DTYPE_MATRIX)
    ...
    # Identity can be followed by anything Delta can be followed by
    ...
    print(f"  Braid group: B_{config.n}")
    print(f"  Representation: ({config.n - config.r}, {config.r}), dimension {dim}")
    print(f"  Number of simples: {num_simples}")
    print(f"  Degree window: [0, {D-1}] ({D} coefficients) - NON-NEGATIVE ONLY")
    
    return simple_matrices, valid_suffixes, num_valid_suffixes
